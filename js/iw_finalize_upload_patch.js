...
async function doConfirm() {
  ...
  if (!name || !linkUrl) {
    alert('Name and Profile URL are required.');
    return;
  }
  ...
  // Étape 1 : Upload de l’image AVANT finalize (si présente)
  try {
    ...
    if (file) {
      ...
      fd.append('uid', uid);
      fd.append('blocks', JSON.stringify(blocks)); // optionnel mais utile
      ...
      console.log('[IW patch] image linked:', imageUrl);
    }
  } catch (e) {
    console.warn('[IW patch] upload failed:', e);
    // On ne bloque pas le processus si upload échoue
  }

  // Étape 2 : Re-réservation (sécurisation)
  try {
    const rsv = await fetchWithJWT('/.netlify/functions/reserve', {
      method: 'POST',
  ...
  // Étape 3 : Finalize AVEC imageUrl si dispo
  try {
    const body = { uid, name, linkUrl, blocks };
    if (imageUrl) body.imageUrl = imageUrl;

    const fRes = await fetchWithJWT('/.netlify/functions/finalize', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
