...
// Assure-toi que jwt-middleware.js se trouve dans le même dossier netlify/functions
...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    // Build a req-like object compatible with requireAuth/getAuthenticatedUID
    const req = {
      method: event.httpMethod,
      headers: event.headers || {},
      json: async () => {
        if (!event.body) return {};
        try { return JSON.parse(event.body); } catch (err) { return typeof event.body === 'object' ? event.body : {}; }
      }
    };

    // Synchronous auth check (requireAuth supports both middleware and sync check)
    ...
    try {
      await ghPutFile(PATH_JSON, newContent, sha, `reserve ${locked.length} blocks by ${uid}`);
    } catch (e) {
      // handle race: retry once by reloading latest state
      console.warn('[reserve] ghPutFile failed, retrying once:', String(e));
      if (String(e).includes('GITHUB_PUT_FAILED')) {
        ...
        await ghPutFile(PATH_JSON, content2, got.sha, `reserve(retry) ${locked2.length} by ${uid}`);
