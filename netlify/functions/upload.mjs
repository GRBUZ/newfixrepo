// netlify/functions/upload.mjs (v2, avec diagnostics + JWT)
...
export default async (req) => {
  try {
    ...
    // Récupération de l'UID authentifié (pour logs/traçabilité)
    ...
    const putBin   = await ghPutBinary(repoPath, buffer, `feat: upload ${filename} for ${regionId} by ${uid}`);
    ...
    const putJson = await ghPutJson(STATE_PATH, state, sha, `chore: set imageUrl for ${regionId} by ${uid}`);
